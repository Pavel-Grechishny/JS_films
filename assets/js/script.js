...
function switchThemeSite(){
    ...
    if(now.getHours() > 18 && now.getHours() < 20) theme.href = 'assets/css/white-theme.css';
}

// скрыть(показать) банер
function removeHalfHeight(){ 
    search.addEventListener('focus', ()=>{if(search.value == "") document.querySelector('.baner').classList.remove('half-height');})
    search.addEventListener('blur', ()=>{if(search.value == "" ) document.querySelector('.baner').classList.add('half-height');})
}